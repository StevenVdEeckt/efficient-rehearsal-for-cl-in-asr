...
class Trainer:
    ...
    def resume(
        ...
        # 3. optimizer
        logging.info(f"keys in the checkpoint: {states.keys()}")
        use_adjusted = 'param_group_to_layer' in states.keys()
        logging.info(f"Using adjusted? {use_adjusted}")
        for optimizer, state in zip(optimizers, states["optimizers"]):
            if use_adjusted:
                adjust_optimizer_state_dict(optimizer, state, model, states['param_group_to_layer'])
            else:
                optimizer.load_state_dict(state)
        if use_adjusted and getattr(model, 'adapter_allocator', None) is not None:
            model.adapter_allocator.set_param_group_to_layer(states['param_group_to_layer'])
        # 4. scheduler
        ...
        # adapter_allocator 
        if "adapter-allocator" in states.keys():
            assert hasattr(model, 'adapter_allocator') 
            assert hasattr(model.adapter_allocator, 'resume_checkpoint')
            model.adapter_allocator.resume_checkpoint(states["adapter-allocator"])
    ...
    def run(
        ...
        # import continual_learning here: it's only needed during training
        # otherwise it causes a circular import error during decoding
        import espnet.nets.pytorch_backend.continual_learning3 as cl
        ...
        # prepare task tree for LoRA
        if getattr(model, 'adapter_allocator', None) is not None and getattr(model.adapter_allocator, 'update_task_tree', None) is not None:
            logging.info("Preparing the task tree...")
            model.adapter_allocator.update_task_tree(model, valid_iter_factory.build_iter(0), options=trainer_options)
        ...
        # pass epoch number to CL method if necessary
        if getattr(model, 'cl_method', None) is not None and hasattr(model.cl_method, 'init_model'):
            model.cl_method.init_model(start_epoch)
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            # if model has Weight-Based CL method, apply it
            if hasattr(model, 'cl_method') and hasattr(model.cl_method, 'update_model'):
                model.cl_method.update_model(model)
            ...
            # save the adapter allocator object if necessary
            #if getattr(model, 'adapter_allocator', None) is not None and start_epoch >= trainer_options.max_epoch + 1:
            #    model.adapter_allocator.save(model, output_dir, True)
    ...
    def train_one_epoch(
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            if iiter % accum_grad == 0:
                ...
                # if model has Gradient-Based CL method, apply it
                if hasattr(model, 'cl_method') and hasattr(model.cl_method, 'consolidate_update'):
                    model.cl_method.consolidate_update(model) 
