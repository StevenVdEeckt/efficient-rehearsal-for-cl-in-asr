...
from espnet2.samplers.sorted_score_batch_sampler import SelectiveBatchSampler
...
class IteratorOptions:
    ...
    train: bool
    batches_per_epoch: Optional[Sequence[float]]
    grouped: bool
...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument("--memory_shape_file", type=str, action="append", default=[])
        group.add_argument("--collect_modes", type=str, default="", help="Modes (train, valid, memory) for collecting stats")
        ...
        group.add_argument(
            ...
            type=int,
            default=0,
            help="Mini-batch size of the memory (if not provided, is equal to training batch size")
        group.add_argument(
            "--memory_grouped",
            type=str2bool,
            default=False,
            help="If true, memory is grouped based on language or task ID")
        ...
        group.add_argument(
            ...
            type=str,
            default=None,
            help="Path to stored dictionary with scores per utterance",
        )
        group.add_argument(
            "--batches_per_epoch",
            default=None,
            help="List of % batches per epoch to use",
        )
        group.add_argument(
            "--multiple_iterator",
            type=str2bool,
        ...
        group.add_argument(
            "--memory_data_path_and_name_and_type",
            type=str2triple_str,
            action="append",
            default=[],
        )
    ...
    def build_optimizers(
        ...
        if args.sharded_ddp:
            ...
            if issubclass(optim_class, NamedOptimizer):
                named_params = [(name, param) for name, param in model.named_parameters()]
                optim = optim_class(named_params, **args.optim_conf)
            elif args.exclude_weight_decay:
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if (
            ...
            for t in args.freeze_param:
                ...
                for k, p in model.named_modules():
                    if (k.startswith(t + ".") or k == t) and isinstance(p, (torch.nn.BatchNorm2d, torch.nn.BatchNorm1d)):
                        p.track_running_stats = False
                        logging.info(f"Setting {k}.track_running_stats = False")

            # Use adapter to finetune the large pre-trained foundation models
            if getattr(args, "use_adapter", False):
                create_adapter(model, args.adapter, args.adapter_conf, args)
        ...
        if args.dry_run:
            ...
            # set memory iter
            if hasattr(model, 'cl_method') and model.cl_method and hasattr(model.cl_method, 'set_loader'):
                if len(args.memory_shape_file) != 0:
                    memory_key_file = args.memory_shape_file[0]
                else:
                    memory_key_file = None

                if args.memory_batch_size < 1:
                    args.memory_batch_size = args.batch_size

                memory_iter=cls.build_streaming_iterator(
                    data_path_and_name_and_type=args.memory_data_path_and_name_and_type,
            ...
            collect_stats(
                model=model.to(device="cuda" if args.ngpu > 0 else "cpu"),
                ...
                memory_iter=memory_iter,
                output_dir=output_dir,
                ngpu=args.ngpu,
                log_interval=args.log_interval,
                write_collected_feats=args.write_collected_feats,
                collect_modes=args.collect_modes,
            )
        else:
            ...
            # update model in CL module (if required)
            if hasattr(model, 'pass_model'):
                model.pass_model(optimizers=optimizers)

            if not os.path.isfile(str(output_dir) + '/initial_model.pth'):
                logging.info('storing at: %s' % (str(output_dir) + '/initial_model.pth'))
                torch.save(model.state_dict(), str(output_dir) + '/initial_model.pth')
            ...
            if hasattr(model, 'cl_method') and model.cl_method and hasattr(model.cl_method, 'set_loader'):
                memory_args = copy.deepcopy(args)
                if args.memory_batch_size >= 1:
                    memory_args.batch_size = args.memory_batch_size

                memory_iter_factory = cls.build_iter_factory(
                        args=memory_args,
                        distributed_option=distributed_option,
                        mode="memory",
                )
                model.cl_method.set_loader(memory_iter_factory)
    ...
    def build_iter_options(
        ...
        if mode == "train":
            ...
            collate_fn = cls.build_collate_fn(args, train=True)
            data_path_and_name_and_type = args.memory_data_path_and_name_and_type
            shape_files = args.memory_shape_file
            #if args.memory_batch_type == "grouped":
            #    shape_files += (args.task_file,)
            batch_size = args.batch_size
            batch_bins = args.batch_bins
            batch_type = args.batch_type #args.memory_batch_type if args.memory_batch_type else args.batch_type
            max_cache_size = args.max_cache_size
            max_cache_fd = args.max_cache_fd
            allow_multi_rates = args.allow_multi_rates
            distributed = distributed_option.distributed
            ...
            num_iters_per_epoch = args.num_iters_per_epoch
            train = True
            grouped = args.memory_grouped == True
            logging.info(f"GROUPED FOR MEMORY = {grouped}")
        ...
        batches_per_epoch = getattr(args, 'batches_per_epoch', None)
    ...
    def build_sequence_iter_factory(
        ...
        if hasattr(args, 'initial_scores'):
            initial_scores = args.initial_scores
        else:
            initial_scores = None
        ...
        return SequenceIterFactory(
            dataset=dataset,
            batches=batch_sampler if isinstance(batch_sampler, SelectiveBatchSampler) else batches,
            seed=args.seed,
            ...
            batches_per_epoch=iter_options.batches_per_epoch,
        )
