...
from espnet2.samplers.sorted_score_batch_sampler import SortedScoreBatchSampler
from espnet2.samplers.unsorted_batch_sampler import UnsortedBatchSampler
from espnet2.samplers.random_percentage_batch_sampler import RandomPercentageBatchSampler
...
def build_batch_sampler(
    ...
    if type == "unsorted":
        ...
        # Initialize scores with a default value (e.g., 0.0)
        if initial_scores is not None:
            initial_scores = torch.load(initial_scores)    
        retval = SortedScoreBatchSampler(
            batch_size=batch_size,
            shape_file=shape_files[0],
    ...
    elif type == "random_selective":
        # Initialize scores with a default value (e.g., 0.0)
        if initial_scores is not None:
            initial_scores = torch.load(initial_scores)
        retval = RandomPercentageBatchSampler(
            batch_size=batch_size,
            shape_file=shape_files[0],
            initial_scores=initial_scores,
            sort_in_batch=sort_in_batch,
