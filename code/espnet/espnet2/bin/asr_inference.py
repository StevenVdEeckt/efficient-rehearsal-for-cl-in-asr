...
class Speech2Text:
    ...
    def __init__(
        ...
        if token_list is None:
            ...
            token_list = load_token_list(token_list)
    ...
    @torch.no_grad()
    def __call__(self, speech: Union[torch.Tensor, np.ndarray], keys=None) -> Union[
        ListOfHypothesis,
        Tuple[
            ListOfHypothesis,
        ...
        if self.task_label is None and keys is not None:
            task_label = keys[0].split("-")[-1].split("_")[-2]
            logging.info(f"Running with task_label = {task_label}")
        else:
        ...
        # b. Forward Encoder
        enc, enc_olens = self.asr_model.encode(**batch, lang_sym=task_label)
        if self.multi_asr:
            enc = enc.unbind(dim=1)  # (batch, num_inf, ...) -> num_inf x [batch, ...]
        # beam_size args
        bs_args = {'lang_sym': task_label}
        if self.enh_s2t_task or self.multi_asr:
            ...
            for spk, enc_spk in enumerate(enc, 1):
                ...
                ret = self._decode_single_sample(enc_spk[0], **bs_args)
            ...
            # c. Passed the encoder result and the beam search
            results = self._decode_single_sample(enc[0], **bs_args)
    ...
    def _decode_single_sample(self, enc: torch.Tensor, **kwargs):
        if self.beam_search_transducer:
            ...
            nbest_hyps = self.beam_search(
                x=enc, maxlenratio=self.maxlenratio, minlenratio=self.minlenratio, **kwargs
            )
...
def inference(
    ...
    task_label: Optional[Union[str, int]],
):
    assert check_argument_types()
    if batch_size > 1:
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            logging.info(f"keys = {keys}, batch = {batch.keys()}")

            # N-best list of (text, token, token_int, hyp_object)
            try:
                results = speech2text(**batch, keys=keys)
...
def get_parser():
    ...
    group.add_argument(
        "--task_label",
        default=None,
        help="Label (or list of labels) of the task to decode")
    group.add_argument(
        "--prompt_token_file",
        type=str,
        default=None,
